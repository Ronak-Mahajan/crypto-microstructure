...
- **Results**: `results/README.md` is generated by `make results` and now
  carries one measured day. `results/self-test/README.md` is the same pipeline over the
  committed synthetic fixture, written by `make smoke`; a banner above
  every table labels it a self-test rather than a measurement, and its
  header names the command that produced it.

The first day is measured, and the headline is that the signal is real and
does not pay for taking.
...
result. *Measured on one Tardis day; the headline is above. More days widen
it, and the recorder supplies the ones Tardis does not serve.*
